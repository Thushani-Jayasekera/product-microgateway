...
GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -v -o target/adapter-linux-amd64 -tags debug github.com/wso2/product-microgateway/adapter/cmd/adapter
GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -v -o target/adapter-linux-arm64 -tags debug github.com/wso2/product-microgateway/adapter/cmd/adapter
if [ $? -ne 0 ]; then 
